...
class SubjectSemestersScreen extends StatefulWidget {
  final String name;
  final int subjectId;

  const SubjectSemestersScreen({
    super.key,
    required this.name,
  ...
  @override
  State<SubjectSemestersScreen> createState() => _SubjectSemestersScreenState();
}

class _SubjectSemestersScreenState extends State<SubjectSemestersScreen> {
  Future<List<Semester>> _getSemestersFuture() async {
    try {
      return await SemesterService.getSemestersBySubject(widget.subjectId);
    } catch (e) {
      throw e.toString();
    }
  }
